...
import { createInitialState, saveState, loadState, clearSave, saveToSlot, loadFromSlot, slotSummary, listSlots, serializeState, deserializeSave, SAVE_SLOT_COUNT, } from "../engine/state.js";
...
import { FLAG_BRIEFING_COMPLETE, FLAG_FORM_COMPLETE, FLAG_PC_ALIAS, FLAG_PC_PROFESSION, FLAG_PC_REAL_NAME, FLAG_CREDITS, FLAG_ARMOURY_CABINET_OPEN, FLAG_SHIPYARD_STRIKES, FLAG_SHIPYARD_PATROL_HEAT, FLAG_SHIPYARD_INTRUDING, FLAG_CHAS_APPROACHED, } from "../world/jackrabbit/flags.js";
import { PATROL_CATCH_AT, STRIKES_TO_BAR, EVASION_HEAT } from "../world/jackrabbit/shipyard_security.js";
import { CHAS_MENACE_LIMIT } from "../world/jackrabbit/bar_npcs.js";
const _ls = {};
...
test("liner: the AetherLink advert panel plays its 'we look after our own' PR (cycling reel, +1/spot)", () => {
    const engine = bootToLiner();
    const before = engine.state.score;
    // First spot: brand. Naming AetherLink is the point of the planting.
    captured.length = 0;
    engine.submit("examine advert");
    assert(/AetherLink/.test(flatOutput()), "an AetherLink spot plays");
    // Second spot: the employee-care PR (the one the later reveal curdles).
    captured.length = 0;
    engine.submit("examine advert");
    assert(/look after our own/i.test(flatOutput()), "the 'we look after our own' employee-care spot");
    // Third spot: the field-operative spot, ending on the tagline.
    captured.length = 0;
    engine.submit("examine advert");
    assert(/No one left behind/i.test(flatOutput()), "the field-operative spot + tagline");
    eq(engine.state.score - before, 3, "+1 for each of the three distinct spots watched");
    // The reel loops back to the brand spot — and re-watching scores nothing more.
    captured.length = 0;
    engine.submit("examine advert");
    assert(/made it our problem/i.test(flatOutput()), "the reel loops back to the first spot");
    eq(engine.state.score - before, 3, "re-watching a spot doesn't score again");
});
...
// --- the shipyard gun trail + the night patrol ---------------------------
test("armoury: the crowbar and the damaged cabinet start in place", () => {
    const engine = bootToHorizon();
    eq(engine.state.itemLocations["crowbar"], "horizon_tool_store", "crowbar waits in the Tool Store");
    eq(engine.state.itemLocations["damaged_cabinet"], "horizon_untheatrical_armory", "cabinet is in the armoury");
});
test("armoury: the cabinet needs the crowbar; prise it and the sidearm is inside", () => {
    const engine = bootAt("horizon_untheatrical_armory");
    // Bare hands: refused with a leverage hint, nothing revealed.
    captured.length = 0;
    engine.submit("open cabinet");
    assert(/lever|purchase|tool/i.test(flatOutput()), "refused without leverage");
    assert(!engine.state.flags[FLAG_ARMOURY_CABINET_OPEN], "cabinet still shut");
    assert(engine.state.itemLocations["sidearm"] === undefined && !engine.state.inventory.includes("sidearm"), "no gun yet");
    // With the crowbar in hand, prise it open — the sidearm is revealed and takeable.
    engine.state.inventory.push("crowbar");
    captured.length = 0;
    engine.submit("use crowbar on cabinet");
    assert(engine.state.flags[FLAG_ARMOURY_CABINET_OPEN], "cabinet prised open");
    assert(/sidearm/i.test(flatOutput()), "the sidearm is revealed");
    engine.submit("take sidearm");
    assert(engine.state.inventory.includes("sidearm"), "took the sidearm");
});
test("shoot: armed, SHOOT CHAS at the Long Shot ends the game (Convict)", () => {
    const engine = bootAt("ez1_long_shot");
    engine.state.ticks = engine.state.dayLength + 5;
    engine.state.isDaytime = false; // night: Chas is in
    engine.state.npcLocations = { ...(engine.state.npcLocations ?? {}), chas: "ez1_long_shot" };
    engine.state.inventory.push("sidearm");
    captured.length = 0;
    engine.submit("shoot chas");
    assert(engine.state.ended, "the game ended");
    eq(engine.state.endingId, "convict", "the Convict ending fired");
    eq(engine.state.survived, true, "the PC survives — imprisoned, not dead");
    assert(/Convict/i.test(flatOutput()), "ranked Convict");
    assert(/Drayton/.test(flatOutput()), "the killing of Drayton's son is narrated");
});
test("shoot: with no weapon, SHOOT does nothing", () => {
    const engine = bootAt("ez1_long_shot");
    engine.state.npcLocations = { ...(engine.state.npcLocations ?? {}), chas: "ez1_long_shot" };
    captured.length = 0;
    engine.submit("shoot chas");
    assert(!engine.state.ended, "no ending");
    assert(/not carrying|not armed|nothing to shoot/i.test(flatOutput()), "told you're unarmed");
});
test("shoot: the gun is single-purpose — the PC won't shoot anyone but Chas", () => {
    const engine = bootAt("ez1_brass_rail");
    engine.state.npcLocations = { ...(engine.state.npcLocations ?? {}), ozzy: "ez1_brass_rail" };
    engine.state.inventory.push("sidearm");
    captured.length = 0;
    engine.submit("shoot ozzy");
    assert(!engine.state.ended && !engine.state.dead, "no ending, no death");
    assert(/lower|not quite that person|not why you came/i.test(flatOutput()), "the PC declines");
});
test("shipyard patrol: caught in the open at NIGHT is an instant exile (a break-in, no strikes)", () => {
    const engine = bootAt("horizon_shipyard_entrance");
    engine.state.ticks = engine.state.dayLength + 5;
    engine.state.isDaytime = false; // solidly night
    engine.state.flags[FLAG_SHIPYARD_INTRUDING] = true; // sneaked in past the gate
    engine.state.flags[FLAG_SHIPYARD_STRIKES] = 0;
    captured.length = 0;
    for (let i = 0; i < PATROL_CATCH_AT; i++)
        engine.submit("wait");
    assert(engine.state.ended, "barred on the spot — a night-time intruder gets no warnings");
    eq(engine.state.endingId, "barred", "the Deported ending fired");
    eq(Number(engine.state.flags[FLAG_SHIPYARD_STRIKES]) || 0, 0, "no strike banked — it's instant");
});
test("shipyard patrol: a single WAIT UNTIL DAYTIME in the open at night rides through to capture", () => {
    // The telegraph is ambient, so the long wait isn't stalled a tick at a time —
    // loitering in the open carries straight on to the (night) exile.
    const engine = bootAt("horizon_shipyard_entrance");
    engine.state.ticks = engine.state.dayLength + 5;
    engine.state.isDaytime = false; // solidly night
    engine.state.flags[FLAG_SHIPYARD_INTRUDING] = true;
    captured.length = 0;
    engine.submit("wait until daytime");
    assert(engine.state.ended, "the open-yard loiterer is caught, not safely parked");
    eq(engine.state.endingId, "barred", "the Deported ending fired");
});
test("shipyard patrol: waiting out the night from cover is safe (no capture)", () => {
    // In a bay (cover), the same long wait should run to dawn unharmed.
    const engine = bootAt("horizon_small_bay_3");
    engine.state.ticks = engine.state.dayLength + 5;
    engine.state.isDaytime = false;
    engine.state.flags[FLAG_SHIPYARD_INTRUDING] = true;
    captured.length = 0;
    engine.submit("wait until daytime");
    assert(!engine.state.ended && !engine.state.dead, "hidden in cover, the night passes without a capture");
    assert(engine.state.isDaytime, "the wait carried through to daytime");
});
test("shipyard patrol: a bolthole resets the heat (no capture)", () => {
    const engine = bootAt("horizon_shipyard_entrance");
    engine.state.ticks = engine.state.dayLength + 5;
    engine.state.isDaytime = false;
    engine.state.flags[FLAG_SHIPYARD_INTRUDING] = true;
    engine.submit("wait");
    engine.submit("wait"); // heat building in the open
    assert((Number(engine.state.flags[FLAG_SHIPYARD_PATROL_HEAT]) || 0) >= 1, "heat is up");
    // Duck into the Tool Store — a bolthole. The heat bleeds off; no strike.
    engine.state.currentRoom = "horizon_tool_store";
    captured.length = 0;
    engine.submit("wait");
    eq(engine.state.flags[FLAG_SHIPYARD_PATROL_HEAT], undefined, "heat reset in cover");
    eq(Number(engine.state.flags[FLAG_SHIPYARD_STRIKES]) || 0, 0, "not caught");
    assert(/out of sight|beam|sweep|passes/i.test(flatOutput()), "the patrol passes by");
});
test("shipyard patrol: caught in the open by DAY (the day crew collar you on sight)", () => {
    const engine = bootAt("horizon_shipyard_entrance");
    engine.state.ticks = 5;
    engine.state.isDaytime = true; // daytime in the yard proper
    engine.state.flags[FLAG_SHIPYARD_INTRUDING] = true; // lingered past dawn
    engine.state.flags[FLAG_SHIPYARD_STRIKES] = 0;
    captured.length = 0;
    engine.submit("wait");
    eq(engine.state.flags[FLAG_SHIPYARD_STRIKES], 1, "collared on sight (a strike)");
    eq(engine.state.currentRoom, "horizon_shipyard_reception", "escorted out");
});
test("shipyard patrol: the third DAYTIME strike bars you from the Outpost (Deported)", () => {
    const engine = bootAt("horizon_shipyard_entrance");
    engine.state.ticks = 5;
    engine.state.isDaytime = true; // daytime — lingered past dawn
    engine.state.flags[FLAG_SHIPYARD_INTRUDING] = true;
    engine.state.flags[FLAG_SHIPYARD_STRIKES] = STRIKES_TO_BAR - 1; // two strikes already on the log
    captured.length = 0;
    engine.submit("wait"); // collared on sight by the day crew — the third strike
    assert(engine.state.ended, "barred — the game is over");
    eq(engine.state.endingId, "barred", "the Deported ending fired");
    eq(engine.state.survived, true, "the PC survives, just exiled");
    assert(/Deported/i.test(flatOutput()), "ranked Deported");
});
test("armoury scoring: crowbar (+1), prising the locker (+1), the gun (+1)", () => {
    const engine = bootAt("horizon_tool_store");
    let before = engine.state.score;
    engine.submit("take crowbar");
    eq(engine.state.score - before, 1, "+1 for the crowbar");
    engine.state.currentRoom = "horizon_untheatrical_armory";
    before = engine.state.score;
    engine.submit("use crowbar on cabinet");
    eq(engine.state.score - before, 1, "+1 for prising the gun locker");
    before = engine.state.score;
    engine.submit("take sidearm");
    eq(engine.state.score - before, 1, "+1 for the gun");
});
test("shoot: killing Chas forfeits everything except the curry (and recalls it)", () => {
    const engine = bootAt("ez1_long_shot");
    engine.state.npcLocations = { ...(engine.state.npcLocations ?? {}), chas: "ez1_long_shot" };
    engine.state.inventory.push("sidearm");
    // Bank some non-curry points AND the curry strand (ate + survived = 10).
    awardScore(engine.state, "took_crowbar", 1);
    awardScore(engine.state, "took_sidearm", 1);
    awardScore(engine.state, "ate_curry", 8);
    awardScore(engine.state, "survived_curry", 2);
    captured.length = 0;
    engine.submit("shoot chas");
    eq(engine.state.score, 10, "only the curry's 8+2 survive the verdict");
    assert(/curry/i.test(flatOutput()), "the cell-curry beat plays for a curry survivor");
});
test("shoot: with no curry eaten, killing Chas wipes the score to zero", () => {
    const engine = bootAt("ez1_long_shot");
    engine.state.npcLocations = { ...(engine.state.npcLocations ?? {}), chas: "ez1_long_shot" };
    engine.state.inventory.push("sidearm");
    awardScore(engine.state, "took_crowbar", 1);
    awardScore(engine.state, "prised_armoury_cabinet", 1);
    captured.length = 0;
    engine.submit("shoot chas");
    eq(engine.state.score, 0, "no curry, no mercy — everything forfeit");
    assert(!/curry/i.test(flatOutput()), "no curry beat without the curry");
});
test("shipyard patrol: slipping a CLOSE patrol into cover scores +4 (once only)", () => {
    const engine = bootAt("horizon_shipyard_entrance");
    engine.state.ticks = engine.state.dayLength + 5;
    engine.state.isDaytime = false;
    engine.state.flags[FLAG_SHIPYARD_INTRUDING] = true;
    for (let i = 0; i < EVASION_HEAT; i++)
        engine.submit("wait"); // heat climbs to EVASION_HEAT
    let before = engine.state.score;
    engine.state.currentRoom = "horizon_tool_store"; // duck into cover
    engine.submit("wait");
    eq(engine.state.score - before, 4, "+4 for slipping a close patrol");
    // A second close evasion earns nothing — once only.
    before = engine.state.score;
    engine.state.currentRoom = "horizon_shipyard_entrance";
    for (let i = 0; i < EVASION_HEAT; i++)
        engine.submit("wait");
    engine.state.currentRoom = "horizon_tool_store";
    engine.submit("wait");
    eq(engine.state.score - before, 0, "no second award");
});
test("shipyard patrol: ducking a DISTANT patrol scores nothing", () => {
    const engine = bootAt("horizon_shipyard_entrance");
    engine.state.ticks = engine.state.dayLength + 5;
    engine.state.isDaytime = false;
    engine.state.flags[FLAG_SHIPYARD_INTRUDING] = true;
    engine.submit("wait");
    engine.submit("wait"); // heat 2 — well short of EVASION_HEAT
    const before = engine.state.score;
    engine.state.currentRoom = "horizon_tool_store";
    engine.submit("wait");
    eq(engine.state.score - before, 0, "too far off to count as an evasion");
});
...
// Drive the Long Shot up to Chas's menacing approach: night, seated, four
// drinks in, AND the late-game gate open (the Burke pivot). Leaves the danger
// window OPEN (approached, undefused, menace n=1).
function seatAndApproachChas(engine) {
    engine.state.currentRoom = "ez1_long_shot";
    engine.state.ticks = engine.state.dayLength + 5;
    engine.state.isDaytime = false;
    engine.state.flags["burke_pivot_done"] = true; // the late-game gate (AetherLink traced + back to Burke)
    engine.submit("wait"); // Chas present at night
    engine.submit("sit");
    for (let i = 0; i < 4; i++)
        engine.submit("buy drink"); // the fourth = the approach
}
test("chas gate: before the Burke pivot, drinking never brings Chas over", () => {
    const engine = bootToHorizon();
    engine.state.currentRoom = "ez1_long_shot";
    engine.state.ticks = engine.state.dayLength + 5;
    engine.state.isDaytime = false;
    ...
    engine.submit("sit");
    for (let i = 0; i < 6; i++)
        engine.submit("buy drink"); // well past the threshold
    assert(!engine.state.flags["chas_approached"], "no approach without the gate");
    assert(!engine.state.scoreHooks.has("chas_gave_intel"), "and so no intel");
    assert(!engine.state.dead && !engine.state.ended, "and no menace/death");
});
test("ask synonyms: TELL works (raising the hunt via TELL defuses Chas + cracks)", () => {
    const engine = bootToHorizon();
    seatAndApproachChas(engine); // window open
    captured.length = 0;
    engine.submit("tell chas about investigation"); // 'tell' routes to 'ask'
    assert(engine.state.scoreHooks.has("chas_gave_intel"), "TELL ... about the hunt defuses + cracks Chas");
    assert(flatOutput().includes("Jack Abbott"), "the real name lands");
    assert(!engine.state.dead, "and raising it keeps you alive");
});
...
test("long shot: drinking brings Chas over MENACING (no intel); raising the Jackrabbit defuses (B4b)", () => {
    const engine = bootToHorizon();
    engine.state.currentRoom = "ez1_long_shot";
    engine.state.ticks = engine.state.dayLength + 5;
    engine.state.isDaytime = false;
    engine.state.flags["burke_pivot_done"] = true; // the late-game gate is open
    engine.submit("wait"); // Chas present at night
    // Ordering before sitting is refused.
    captured.length = 0;
    engine.submit("buy drink");
    assert(flatOutput().toLowerCase().includes("table first"), "you have to sit first");
    // Sit, then drink up — Chas comes over on the fourth, to WEIGH you, not gossip.
    engine.submit("sit");
    engine.submit("buy drink");
    engine.submit("buy drink");
    engine.submit("buy drink");
    captured.length = 0;
    engine.submit("buy drink"); // the fourth: the approach
    assert(engine.state.flags["chas_approached"] === true, "the approach opens the window");
    assert(!flatOutput().includes("Jack Abbott"), "the menacing approach hands over NO intel");
    assert(!engine.state.scoreHooks.has("chas_gave_intel"), "no crack on the approach");
    // The defuse: raise the Jackrabbit -> the spite-crack (+4) and you live.
    const before = engine.state.score;
    captured.length = 0;
    engine.submit("ask chas about jackrabbit");
    assert(flatOutput().includes("Jack Abbott"), "the defuse spills the name");
    assert(flatOutput().toLowerCase().includes("squirt"), "the canon 'nasty little squirt'");
    assert(engine.state.scoreHooks.has("chas_gave_intel"), "the crack scores via the defuse");
    eq(engine.state.score, before + 4, "+4 for the intel");
    assert(!engine.state.dead, "raising the Jackrabbit keeps you alive");
});
...
test("strand 2: the analysis finishes (datapad notification), then reads out at a terminal", () => {
    ...
    assert(/public terminal|time enough/i.test(flatOutput()), "an in-game nudge to go read it (no datapad ping)");
...
test("burke: night-only — sleep is refused (turned out), and daytime ushers you out", () => {
    const engine = bootToHorizon();
    engine.state.currentRoom = "horizon_burke_s_workshop";
    engine.state.ticks = engine.state.dayLength + 5;
    engine.state.isDaytime = false; // night
    captured.length = 0;
    engine.submit("sleep"); // you don't kip in Burke's workshop
    eq(engine.state.currentRoom, "horizon_narrow_corridor8", "turned straight out to the corridor");
    assert(!engine.state.isDaytime, "no time slept away — still night");
    assert(/doss-house|none of that/i.test(flatOutput()), "Burke's refusal line landed");
    // Lingering inside into the day still ushers you out (burkeWorkshopDayTick).
    engine.state.currentRoom = "horizon_burke_s_workshop";
    engine.state.ticks = 5;
    engine.state.isDaytime = true; // daytime (tick re-derives the phase)
    captured.length = 0;
    engine.submit("wait");
    eq(engine.state.currentRoom, "horizon_narrow_corridor8", "daytime in the workshop ushers you out");
    assert(flatOutput().toLowerCase().includes("daylight"), "Burke's daybreak eject line landed");
});
...
test("chas: contempt before the build-up; the spite-crack is the defuse during the menace (+4)", () => {
    const engine = bootToHorizon();
    engine.state.currentRoom = "ez1_long_shot";
    engine.state.ticks = engine.state.dayLength + 5;
    engine.state.isDaytime = false;
    engine.submit("wait");
    // Pre-approach: talk scores; the jackrabbit topic gives contempt but no name,
    // and crucially no intel — the hard intel is now gated behind the menace.
    captured.length = 0;
    engine.submit("talk chas");
    assert(engine.state.scoreHooks.has("talked_to_chas"), "first conversation scores");
    captured.length = 0;
    engine.submit("ask chas about jackrabbit");
    assert(flatOutput().toLowerCase().includes("little nobody"), "contempt, no detail");
    assert(!flatOutput().toLowerCase().includes("abbott"), "no name before the build-up");
    assert(!engine.state.scoreHooks.has("chas_gave_intel"), "no intel without the menace window");
    // The build-up opens the window; now raising the hunt is the DEFUSE -> the crack.
    engine.state.flags["burke_pivot_done"] = true; // the late-game gate is open
    engine.submit("sit");
    for (let i = 0; i < 4; i++)
        engine.submit("buy drink"); // the approach
    const before = engine.state.score;
    captured.length = 0;
    engine.submit("ask chas about hunting");
    assert(engine.state.scoreHooks.has("chas_gave_intel"), "the crack scores (the defuse)");
    ...
    assert(!engine.state.dead, "and the defuse saves you");
    // Idempotent.
    const sc = engine.state.score;
    engine.submit("ask chas about hunting");
    eq(engine.state.score, sc, "the crack fires only once");
});
test("chas menace: running out the clock springs the trap (mischief death, no epilogue)", () => {
    const engine = bootToHorizon();
    seatAndApproachChas(engine); // window open; menace at n=1
    captured.length = 0;
    // Dither without raising the Jackrabbit: every time-costing turn advances it.
    for (let i = 0; i <= CHAS_MENACE_LIMIT && !engine.state.dead; i++)
        engine.submit("wait");
    assert(engine.state.dead, "the trap springs when the window runs out");
    assert(!engine.state.ended, "it's a mischief death, not a narrative ending");
    assert(/Drayton's people|service airlock/i.test(flatOutput()), "the predecessor's exact death");
});
test("chas menace: trying to LEAVE (west) springs the trap (and you never reach the door)", () => {
    const engine = bootToHorizon();
    seatAndApproachChas(engine);
    captured.length = 0;
    engine.submit("west"); // bolt for the only door
    assert(engine.state.dead, "leaving with the window open is fatal");
    eq(engine.state.currentRoom, "ez1_long_shot", "you never make the door");
    assert(/better place|Drayton's people/i.test(flatOutput()), "the intercept, then the death");
});
test("chas menace: STANDING to leave springs the trap", () => {
    const engine = bootToHorizon();
    seatAndApproachChas(engine);
    captured.length = 0;
    engine.submit("stand");
    assert(engine.state.dead, "standing to go is fatal mid-window");
});
test("chas menace: raising the Jackrabbit defuses — then you may leave freely", () => {
    const engine = bootToHorizon();
    seatAndApproachChas(engine);
    engine.submit("ask chas about jackrabbit"); // the defuse
    assert(engine.state.flags["chas_defused"] === true, "the window is closed");
    assert(!engine.state.dead, "alive");
    captured.length = 0;
    engine.submit("stand");
    engine.submit("west");
    assert(!engine.state.dead, "free to leave once defused");
    eq(engine.state.currentRoom, "ez1_brass_rail", "and you actually make it out");
});
...
test("teng: atmospheric until the defect pathway, then the berth pointer (+2)", () => {
    ...
    assert(/shipyard|hold|stow/i.test(flatOutput()), "points at the shipyard hauler / stow-away");
...
test("save/load: the datacard's world `load` command must not shadow the slot picker", () => {
    const engine = bootToHorizon();
    engine.state.currentRoom = "horizon_blue_sector_concourse"; // a distinctive room
    // SAVE → guided picker: slot 1 → name.
    engine.submit("save");
    engine.submit("1");
    engine.submit("my game");
    engine.state.currentRoom = "horizon_arrival_concourse"; // move away
    captured.length = 0;
    // Bare LOAD must open the slot picker (not the datacard handler).
    engine.submit("load");
    assert(flatOutput().toLowerCase().includes("which slot"), "`load` opens the slot picker");
    engine.submit("1");
    eq(engine.state.currentRoom, "horizon_blue_sector_concourse", "state was actually restored");
    // `restore` (the synonym) must reach the picker too.
    engine.state.currentRoom = "horizon_arrival_concourse";
    captured.length = 0;
    engine.submit("restore");
    assert(flatOutput().toLowerCase().includes("which slot"), "`restore` opens the picker too");
    engine.submit("1");
    eq(engine.state.currentRoom, "horizon_blue_sector_concourse", "`restore` also restored the save");
...
// --- Save system: load reconciliation, slots, export/import --------------
test("load reconcile: a stale Chas menace doesn't kill you after loading elsewhere", () => {
    // The reported bug: a save with the Chas menace window "open" (approached,
    // not defused) but the PC at the concourse — loading it must stand the
    // encounter down, not spring the trap on the next ticking turn.
    const engine = bootToHorizon();
    engine.state.currentRoom = "horizon_arrival_concourse";
    engine.state.isDaytime = false;
    engine.state.flags[FLAG_CHAS_APPROACHED] = true; // window open
    saveToSlot(3, "before the bar", "Arrival Concourse", engine.state);
    delete engine.state.flags[FLAG_CHAS_APPROACHED];
    engine.submit("load");
    engine.submit("3");
    eq(engine.state.currentRoom, "horizon_arrival_concourse", "loaded the concourse save");
    assert(!engine.state.flags[FLAG_CHAS_APPROACHED], "the stale menace flag was reconciled away");
    // Exactly the transcript that exposed the bug: ask Barty, then survive.
    captured.length = 0;
    engine.submit("ask barty about food");
    assert(flatOutput().toLowerCase().includes("straight east"), "Barty answers (the turn resolves)");
    assert(!engine.state.dead, "the ticking turn after the load does NOT trigger the Chas death");
});
test("load reconcile: a save taken IN the Long Shot keeps the live menace", () => {
    const engine = bootToHorizon();
    engine.state.currentRoom = "ez1_long_shot";
    engine.state.flags[FLAG_CHAS_APPROACHED] = true;
    saveToSlot(1, "in the bar", "The Long Shot", engine.state);
    engine.submit("load");
    engine.submit("1");
    assert(engine.state.flags[FLAG_CHAS_APPROACHED], "the menace stays open when you load at the bar");
});
test("slots: save to several slots, list them, load one back", () => {
    const engine = bootToHorizon();
    engine.state.currentRoom = "horizon_blue_sector_concourse";
    engine.submit("save");
    engine.submit("2");
    engine.submit("blue save");
    engine.state.currentRoom = "horizon_arrival_concourse";
    engine.submit("save");
    engine.submit("5");
    engine.submit("dock save");
    const slots = listSlots();
    eq(slots.length, SAVE_SLOT_COUNT, "ten slots reported");
    eq(slots[0], null, "slot 1 empty");
    assert(slots[1]?.name === "blue save", "slot 2 named");
    assert(slots[4]?.name === "dock save", "slot 5 named");
    captured.length = 0;
    engine.submit("load");
    const list = flatOutput();
    assert(list.includes("blue save") && list.includes("dock save"), "picker lists both saves");
    engine.submit("2");
    eq(engine.state.currentRoom, "horizon_blue_sector_concourse", "loaded slot 2");
});
test("slots: loading an empty slot re-prompts rather than loading", () => {
    const engine = bootToHorizon();
    const room = engine.state.currentRoom;
    engine.submit("load");
    captured.length = 0;
    engine.submit("4"); // empty slot
    assert(flatOutput().toLowerCase().includes("empty"), "empty slot reported");
    eq(engine.state.currentRoom, room, "still in the same room");
    engine.submit("cancel");
    assert(!engine.inModalMode(), "picker dismissed on cancel");
});
test("slots: saving over an occupied slot asks to confirm", () => {
    const engine = bootToHorizon();
    engine.submit("save");
    engine.submit("1");
    engine.submit("first");
    ...
    captured.length = 0;
    engine.submit("1");
    assert(flatOutput().toLowerCase().includes("overwrite"), "overwrite confirmation prompted");
    engine.submit("y");
    engine.submit("second");
    assert(slotSummary(1)?.name === "second", "slot 1 overwritten with the new name");
});
test("slots: a legacy single-slot save migrates into slot 1", () => {
    const engine = bootToHorizon(); // clears all slots
    engine.state.currentRoom = "horizon_blue_sector_concourse";
    localStorage.setItem("jackrabbit-save-1", serializeState(engine.state, "Recovered save", "Blue Sector"));
    const slots = listSlots();
    assert(slots[0]?.name === "Recovered save", "slot 1 populated from the legacy key");
    eq(localStorage.getItem("jackrabbit-save-1"), null, "legacy key retired after migration");
    eq(loadFromSlot(1)?.currentRoom, "horizon_blue_sector_concourse", "legacy save restored into slot 1");
});
test("export/import: the portable envelope round-trips and reconciles on import", () => {
    const engine = bootToHorizon();
    engine.state.currentRoom = "horizon_arrival_concourse";
    engine.state.isDaytime = false;
    engine.state.flags[FLAG_CHAS_APPROACHED] = true; // a stale menace again
    engine.state.score = 36;
    const json = serializeState(engine.state, "carry me", "Arrival Concourse");
    const back = deserializeSave(json);
    assert(back, "envelope parses");
    eq(back.state.score, 36, "score survives the round-trip");
    assert(back.state.scoreHooks instanceof Set, "scoreHooks rebuilt as a Set");
    const ok = engine.loadSnapshot(json);
    assert(ok, "loadSnapshot accepted the file");
    assert(!engine.state.flags[FLAG_CHAS_APPROACHED], "import reconciled the stale menace");
    captured.length = 0;
    engine.submit("wait");
    assert(!engine.state.dead, "no stale death after import");
});
test("export/import: the EXPORT and IMPORT verbs invoke the UI callbacks", () => {
    clearSave();
    let exportedFilename = "";
    let exportedJson = "";
    let importAsked = false;
    const engine = new GameEngine(jackrabbitWorld, {
        render: () => { }, updateHeader: () => { },
        requestExport: (filename, json) => { exportedFilename = filename; exportedJson = json; },
        requestImport: () => { importAsked = true; },
    });
    engine.state.currentRoom = "horizon_arrival_concourse";
    engine.submit("export");
    assert(exportedFilename.endsWith(".jrsave"), "EXPORT invoked requestExport with a .jrsave filename");
    assert(deserializeSave(exportedJson), "the exported JSON is a valid save");
    engine.submit("import");
    assert(importAsked, "IMPORT invoked requestImport");
});
test("save: transient pending flags never reach a save; durable __-flags do", () => {
    const engine = bootToHorizon();
    engine.state.flags["__pendingGoto"] = "somewhere";
    engine.state.flags["__playerNoteSeq"] = 3;
    const back = deserializeSave(serializeState(engine.state, "x", "x"));
    assert(back, "parses");
    assert(back.state.flags["__pendingGoto"] === undefined, "transient pending flag stripped");
    eq(back.state.flags["__playerNoteSeq"], 3, "durable __-flag preserved");
});
...
test("leash: the AetherLink datapad can't be dropped (it never leaves the PC's hands)", () => {
    const engine = bootToHorizon();
    assert(engine.state.inventory.includes("datapad"), "the PC carries the datapad");
    captured.length = 0;
    engine.submit("drop datapad");
    assert(engine.state.inventory.includes("datapad"), "still held — the drop is refused");
    assert(!/transponder|leash|tracker/i.test(flatOutput()), "and the refusal gives nothing away");
});
...
// --- Endgame: the contract clock + the departures (B7/B8) ----------------
test("contract clock: anchored on arrival; lapses past the deadline (opens home, doesn't end)", () => {
    const engine = bootToHorizon();
    const start = engine.state.flags["contract_start_tick"];
    assert(typeof start === "number", "the clock is anchored at arrival");
    assert(!engine.state.flags["contract_expired"], "not lapsed yet");
    engine.state.ticks = start + 1000000; // well past any deadline
    captured.length = 0;
    engine.submit("wait");
    assert(engine.state.flags["contract_expired"], "the contract has lapsed");
    assert(/engagement closed|lapsed|escrow/i.test(flatOutput()), "the cancellation notice fired");
    assert(!engine.state.ended && !engine.state.dead, "but expiry doesn't end the game");
});
test("submit: files the report (banks the payout) without ending the game", () => {
    const engine = bootToHorizon();
    captured.length = 0;
    engine.submit("submit");
    assert(/yes\s*\/\s*no/i.test(flatOutput()), "a confirm prompt");
    assert(!engine.state.flags["report_submitted"], "not filed until confirmed");
    engine.submit("yes");
    assert(engine.state.flags["report_submitted"] === true, "report filed");
    assert(!engine.state.ended, "filing doesn't end the run");
});
test("loyal ending: SUBMIT, then DEPART home from the concourse — survive, paid (severance closer)", () => {
    const engine = bootToHorizon();
    engine.submit("submit");
    engine.submit("yes");
    engine.state.currentRoom = "horizon_arrival_concourse";
    captured.length = 0;
    engine.submit("depart");
    assert(engine.state.ended, "the run ends at the homebound berth");
    eq(engine.state.endingId, "loyal", "the Loyal ending");
    eq(engine.state.survived, true, "you survive");
    assert(/severed|never know you existed/i.test(flatOutput()), "the severance reveal closes it");
});
test("timeout->home: lapsed + unsubmitted, DEPART home — survive, ranked Failure", () => {
    const engine = bootToHorizon();
    engine.state.flags["contract_expired"] = true; // lapsed, never submitted
    engine.state.currentRoom = "horizon_arrival_concourse";
    captured.length = 0;
    engine.submit("depart");
    eq(engine.state.endingId, "timeout_home", "Timeout -> home");
    eq(engine.state.survived, true, "survive");
    assert(/Rank: Failure/i.test(flatOutput()), "forced rank Failure");
});
test("depart: refused with a live, unsubmitted contract (and away from the docks)", () => {
    const engine = bootToHorizon();
    engine.state.currentRoom = "horizon_arrival_concourse";
    captured.length = 0;
    engine.submit("depart");
    assert(!engine.state.ended, "you can't just bail mid-contract");
    assert(/contract to discharge/i.test(flatOutput()), "told to finish the job first");
});
test("depart: blocked once you've defected (FILE still works, but no berth home)", () => {
    const engine = bootToHorizon();
    engine.state.flags["defecting"] = true;
    engine.submit("submit");
    engine.submit("yes"); // filing is still allowed (it just never arrives)
    assert(engine.state.flags["report_submitted"] === true, "FILE still works after defecting");
    engine.state.currentRoom = "horizon_arrival_concourse";
    captured.length = 0;
    engine.submit("depart");
    assert(!engine.state.ended, "a defector can't take the homebound berth");
    assert(/took that road|other direction|made your choice/i.test(flatOutput()), "the road-closed refusal");
});
test("defect ending: stow away on the shipyard hauler — off-vector death + the cold coda", () => {
    const engine = bootToHorizon();
    engine.state.flags["defecting"] = true;
    engine.state.currentRoom = "horizon_large_bay_2";
    captured.length = 0;
    engine.submit("stow away");
    assert(engine.state.ended, "the run ends");
    eq(engine.state.endingId, "defect", "the Defect ending");
    eq(engine.state.survived, false, "you die off-vector");
    assert(/interceptor|deviation|never know you were/i.test(flatOutput()), "the cold coda plays");
});
test("timeout->elsewhere: stow away after the deadline — off-vector death", () => {
    const engine = bootToHorizon();
    engine.state.flags["contract_expired"] = true;
    engine.state.currentRoom = "horizon_large_bay_2";
    engine.submit("stow away");
    eq(engine.state.endingId, "timeout_elsewhere", "Timeout -> elsewhere");
    eq(engine.state.survived, false, "death");
});
test("stow away: refused when there's no reason to run (no defect, contract still live)", () => {
    const engine = bootToHorizon();
    engine.state.currentRoom = "horizon_large_bay_2";
    captured.length = 0;
    engine.submit("stow away");
    assert(!engine.state.ended && !engine.state.dead, "no meaningless death");
    assert(/not a stowaway|contract that isn't finished/i.test(flatOutput()), "stepped back from it");
});
// --- Captain Hale: Jack's flight instructor (Strand-1 wall) ---------------
// Put the PC on the Simulator Deck and let a tick place/clear Hale for the phase.
function atSimDeck(engine, daytime) {
    engine.state.currentRoom = "training_sim_deck";
    engine.state.ticks = daytime ? 5 : engine.state.dayLength + 5;
    engine.state.isDaytime = daytime;
    engine.submit("wait"); // haleTick runs -> places or clears Hale
}
test("hale: instructs on the Simulator Deck by day, absent at night", () => {
    const engine = bootToHorizon();
    atSimDeck(engine, true);
    captured.length = 0;
    engine.submit("look");
    assert(/Captain Hale/.test(flatOutput()), "present on the deck by day");
    atSimDeck(engine, false);
    captured.length = 0;
    engine.submit("look");
    assert(!/Captain Hale/.test(flatOutput()), "gone once the school goes dark");
});
test("hale: talk scores (+1) + files a note; a curt, unforthcoming greeting", () => {
    const engine = bootToHorizon();
    atSimDeck(engine, true);
    captured.length = 0;
    engine.submit("talk hale");
    assert(engine.state.scoreHooks.has("talked_to_hale"), "the encounter scores");
    assert(/not one of mine/i.test(flatOutput()), "the curt military greeting");
    assert(engine.state.notes.some(n => n.id === "hale_encounter"), "encounter note filed");
});
test("hale: the loyalty wall — raising the boy scores (+2) but yields NO intel", () => {
    const engine = bootToHorizon();
    atSimDeck(engine, true);
    engine.submit("talk hale"); // +1 talked
    const before = engine.state.score;
    captured.length = 0;
    engine.submit("ask hale about jack");
    assert(engine.state.scoreHooks.has("hale_wall"), "the wall beat scores");
    eq(engine.state.score - before, 2, "+2 for drawing out the wall");
    assert(!/abbott/i.test(flatOutput()), "he gives no name");
    assert(!engine.state.flags["jack_real_name"] && !engine.state.flags["jackrabbit_is_ship"], "and sets no Strand-1 intel flags — he's a true dead end");
    assert(engine.state.notes.some(n => n.id === "hale_wall"), "the wall note is filed");
});
test("hale: taught a Drayton, and the money bought no favour (canon link)", () => {
    const engine = bootToHorizon();
    atSimDeck(engine, true);
    captured.length = 0;
    engine.submit("ask hale about drayton");
    assert(/pass mark|whose son|wasted a long climb/i.test(flatOutput()), "fair to all; immune to the money");
});
// --- summary ------------------------------------------------------------
setTimeout(() => {
    console.log("");
